...
p_init[p_init .< 0.1] .= 0
p_init[p_init .> 0.1] .= init_value+rand(1)[1]*0.1
...
np_init = ones(gridap.np+1) * sin(30/180*π) * ω2 * nf
np_init[1:end-1] = p_init[:]
...
for bi = 1 : 6
    ...
    # else
    #     g_opt, p_opt = g0_pkb1_optimize([], theta1, 1e-12, 70; phys1, phys2, control, gridap, usat, damp)
    # end
